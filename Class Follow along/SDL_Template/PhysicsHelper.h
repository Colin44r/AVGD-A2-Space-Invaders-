...
inline bool BoxVsBoxCollision(BoxCollider* b1, BoxCollider* b2) {
	...
	projAxes[2] = (b1->GetVertexPos(0) - b1->GetVertexPos(1)).Normalized();
	projAxes[3] = (b1->GetVertexPos(0) - b1->GetVertexPos(2)).Normalized();
...
inline bool ColliderVsColliderCheck(Collider* c1, Collider* c2) {
	...
	else if (c1->GetType() == Collider::ColliderType::Circle &&
